...
Static members are used in `com.airtribe.learntrack.com.airtribe.learntrack.util.IdGenerator`.
...
* **Benefit:** If these were instance variables, every time we created a `new IdGenerator()`, the counter would reset to 0. Making them `static` ensures they belong to the class, not a specific object, guaranteeing unique IDs for every new com.airtribe.learntrack.entity.
...
* **com.airtribe.learntrack.Main (UI):** Handles user input.
